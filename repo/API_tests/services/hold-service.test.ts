...
describe('HoldService', () => {
  ...
  it('places a hold on selectable seat', async () => {
    ...
    const hold = await svc.placeSeatHold('t1', seat.id, 'u1');
  ...
  it('rejects hold on non-selectable seat', async () => {
    ...
    await seatMapRepo.save(seat);
    await expect(svc.placeSeatHold('t1', seat.id, 'u1')).rejects.toThrow('not selectable');
  });

  it('rejects hold on already-held seat', async () => {
    const seat = createSeatMapEntry({ tripId: 't1', row: 1, number: 1 });
    await seatMapRepo.save(seat);
    await svc.placeSeatHold('t1', seat.id, 'u1');
    await expect(svc.placeSeatHold('t1', seat.id, 'u2')).rejects.toThrow('already held');
  });
  ...
  it('releases a hold', async () => {
    ...
    await svc.releaseSeatHold(hold.id, 'u1');
  ...
  it('release rejects non-owner', async () => {
    ...
    await seatMapRepo.save(seat);
    const hold = await svc.placeSeatHold('t1', seat.id, 'u1');
    await expect(svc.releaseSeatHold(hold.id, 'u2')).rejects.toThrow('owner');
  });
  ...
  it('confirms a hold', async () => {
    ...
    const confirmed = await svc.confirmSeatHold(hold.id, 'u1');
  ...
  it('confirm rejects expired hold', async () => {
    ...
    await expect(svc.confirmSeatHold(hold.id, 'u1')).rejects.toThrow('expired');
  });
  ...
  it('getActiveHoldsForTrip excludes expired', async () => {
    ...
    const h1 = await svc.placeSeatHold('t1', s1.id, 'u1');
    await svc.placeSeatHold('t1', s2.id, 'u2');
  ...
  it('getConfirmedSeatIds returns confirmed seat IDs', async () => {
    ...
    await seatMapRepo.save(seat);
    const hold = await svc.placeSeatHold('t1', seat.id, 'u1');
    await svc.confirmSeatHold(hold.id, 'u1');
  ...
  it('rejects operations on nonexistent seat/hold', async () => {
    await expect(svc.placeSeatHold('t', 'bogus', 'u')).rejects.toThrow('not found');
    await expect(svc.releaseSeatHold('bogus', 'u')).rejects.toThrow('not found');
    await expect(svc.confirmSeatHold('bogus', 'u')).rejects.toThrow('not found');
  });
  ...
  it('auto-expires stale hold when placing new one', async () => {
    ...
    const newHold = await svc.placeSeatHold('t1', seat.id, 'u2');
